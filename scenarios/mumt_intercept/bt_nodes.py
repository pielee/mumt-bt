...
★ 설계 근거 (폐루프 검증, 2026-07-08):
  - 롤 2법칙 블렌드+선회 적분기(Controller_CY GetStickStation) 전제:
    tail-chase가 적기 뒤 standoff에 수렴(거리오차 9.2m RMS), LOS 7.5° 평균
    → 방아쇠(5°/15°) 조건 개폐 확인. 적분기 없으면 P-droop ~22°로 사격 불가.
  - 적기 원 선회 = 8점 웨이포인트 순회 (반경 2km에서 오차 113m RMS, 검증됨).
  - 추격 속도 상한 265: 선회반경 기하 고려 (высок속도 = 큰 선회원 = 수렴 불가).
"""
...
class OrbitPoint(_TakeoffMixin, ActionWithROSTopic):
    ...
    def _build_message(self, agent, blackboard):
        ...
        msg.target_x, msg.target_y, msg.target_z = float(vx), float(vy), float(vz)
...
class InterceptTarget(_TakeoffMixin, ActionWithROSTopic):
    ...
    def _build_message(self, agent, blackboard):
        ...
        gun = False
        to = self._takeoff_update(agent, own)
        if to is not None:                       # 지상 → 먼저 이륙
            vx, vy, vz, speed = to
        elif not enemies:                        # 적 전멸 → SUCCESS
            self._done = True
            if self._last_msg is not None:
                self._last_msg.gun_firing = False
                return self._last_msg
            vx, vy, vz = own.get("x", 0.0), own.get("y", 0.0), own.get("z", 0.0)
            speed = 200.0
        else:
            self._done = False
            ...
            # tail-chase: 적기 자체를 조준 (후방 수렴·사격 기하는 제어기+속도법칙이 형성)
            vx, vy, vz = target.get("x", 0.0), target.get("y", 0.0), target.get("z", 0.0)
            speed = clamp(espd + self._kp * (dist - self._standoff),
            ...
            agent.ros_bridge.node.get_logger().info(
                ...
                f"방위오차={brg_err:.1f}° hp={target.get('hp', '?')} Vtgt={speed:.0f} | "
